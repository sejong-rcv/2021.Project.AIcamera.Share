...
# from models.hrnet import get_cls_net
###############################################################################
# Functions
###############################################################################
def weights_init(m):
...
def define_G(opt, norm='instance', use_noise=False, gpu_ids=[]):    
    # norm_layer = get_norm_layer(norm_type=norm)     
    # netG = SIGGRAPHGenerator(3, 3,norm_layer, use_noise = use_noise, use_tanh=True)
    # netG = get_cls_net(opt)
    netG = build_model(opt, opt.config)
    print(netG)
