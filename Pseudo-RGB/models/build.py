...
def build_model(opt, config):
    ...
    if model_type == "hrnet":
        model = HighResolutionNet(
            opt, config.MODEL.HRNET, num_classes=config.MODEL.NUM_CLASSES
        )

    elif model_type == "hrt":
        model = HighResolutionTransformer(
            opt, config.MODEL.HRT, num_classes=config.MODEL.NUM_CLASSES
        )
        # model.init_weights(pretrained="pretrained/hrt_tiny.pth")
