...
from models.HR_Decoder import HRDecoder
...
class HighResolutionTransformer(nn.Module):
    def __init__(self, opt, cfg, num_classes=1000, **kwargs):
        super(HighResolutionTransformer, self).__init__()

        self.input_nc = opt.input_nc
        self.output_nc = opt.output_nc
        self.isdecoder = opt.isdecoder
        self.num_ch_enc = [ 64, 18, 36, 72, 144 ]
        self.conv1 = nn.Conv2d(self.input_nc, 64, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        in_channels = sum(cfg['STAGE4']['NUM_CHANNELS'])

        if not self.isdecoder:
            self.aux_head = nn.Sequential(
                ...
                nn.Conv2d(
                    ...
                    kernel_size=1,
                    stride=1,
                    padding=1 if 3 == 1 else 0)
            )
        else:
            self.HRDecoder = HRDecoder(num_ch_enc = self.num_ch_enc, scales=range(1),
                                    num_output_channels=opt.output_nc, mobile_encoder=False)
    ...
    def _make_stage(
        ...
        self.feats = {'stage1':[],'stage2':[],'stage3':[], 'stage4':[]}
        self.f_keys = list(self.feats.keys())
        modules = []
    ...
    def forward(self, x_):
        ...
        if not self.isdecoder:
            ...
            return self.HRDecoder(features+mixed_features)
    ...
    #     #     self.feats[self.f_keys[i]].append(feat)
    #     # print(f'stage2 : {len(y_list)}')
    ...
    def init_weights(
        ...
        if os.path.isfile(pretrained):
            ...
            pt_dict = pretrained_dict['model'].keys()
            ...
            for k in pt_dict:
                if k in model_dict.keys():
                    ...
                    outout += 1
            print(f'inin : {len(in_keys)}, outout : {outout}')

            for i in range(len(in_keys)):
                model_dict[in_keys[i]] = pretrained_dict['model'][in_keys[i]]
