...
OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=7 python train.py --height 448 --width 512 --scheduler_step_size 14 --dataset kaist --split kaist --num_epochs 50 --max_depth 50 \
                --batch_size 4 --frame_ids 0 --use_stereo --model_name Kaist_transdssl_RGB\
                --data_path MTN_data --log_dir ./tmp --using_attention --using_diff --model transdssl  \
                --load_weights_folder transdssl_pre #--debug #--num_workers 0
# OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=2 python train.py --height 448 --width 512 --scheduler_step_size 14 --dataset kaist --split kaist --num_epochs 50 --max_depth 50 \
