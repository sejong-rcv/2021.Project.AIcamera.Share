...
def _hrnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        print("Load_weight: ",arch)
        hrnet_pre=timm.create_model('hrnet_w18', pretrained=True)
        hrnet_pre_state_dict=hrnet_pre.state_dict()
        model.load_state_dict({k: v for k,v in hrnet_pre_state_dict.items() if k in model.state_dict()})
    return model
