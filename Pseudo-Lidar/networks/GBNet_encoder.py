...
class GBNetEncoder(nn.Module):
    def __init__(self,opt):
        super(GBNetEncoder, self).__init__()
        self.opt=opt
        self.scales=self.opt.scales
        num_ch_enc= np.array([ 64, 18, 36, 72, 144 ])
        self.encoder1 = hrnet18(True)
...
#         import pdb;pdb.set_trace()
        self.outputs = {}
        input_features=self.encoder1(input_)
...
#                 self.outputs[("disp", i)]=disp1[("disp", i)]*(0.5)+disp2[("disp", i)]*(0.3)+disp3[("disp", i)]*(0.2)
    ...
    def load_kitti_weight(self,path):
        ...
        self.encoder1=self.load_weight(self.encoder1,encoder_statedict)
